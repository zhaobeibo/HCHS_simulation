...
%macro impute_sudaan(start=1, end=100, corr=exchangeable, rr=glm, nimpute=5);
  %do i = &start. %to &end.;
			data samp;
		...
		* combine estimates;
		...
		run;			
		data dt_betas.betas_mi_&corr._&rr._&i;
...
* glm;
* %impute_sudaan();
%impute_sudaan(start = 61, corr=independent);

* NRadj;
%impute_sudaan(rr=NRadj);
%impute_sudaan(corr=independent, rr=NRadj);
