...
run;
	...
	/*============================================================*
	 ...
	 |    - first keep only subjects with observed V3 record       |
	 |    - then remove missing visit records among those subjects |
	 *============================================================*/

	/* identify subjects whose Visit 3 record is observed */
	proc sql;
	  create table _v3_observed_subids_&i as
	  select distinct subid
	  from samplemiss_&i._raw1
	  where v_num = 3 and miss_ind_mar = 0;
	quit;

	proc sort data=samplemiss_&i._raw1;
	  by subid;
	run;

	proc sort data=_v3_observed_subids_&i;
	  by subid;
	run;

	data samplemiss_&i;
	  merge samplemiss_&i._raw1(in=a)
	        _v3_observed_subids_&i(in=b);
	  by subid;

	  if a and b;              /* keep all V1/V2/V3 records for V3-observed subjects */
	  if miss_ind_mar = 0;     /* then remove missing visit records */
...
%let datapath = J:\HCHS\STATISTICS\GRAS\Beibo\Computing Requests\HCHS_simulation\data\sample\sample_miss20pct\;
libname missout "J:\HCHS\STATISTICS\GRAS\Beibo\Computing Requests\HCHS_simulation\data\betas\s6_20miss\";
