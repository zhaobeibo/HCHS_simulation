...
proc printto log = "&homepath./logs/s4_sudaan_miglm_&sysdate..log"
             print= "&homepath./lst/s4_sudaan_miglm_&sysdate..lst" new; 
run; 
...
%macro impute_sudaan(start=, end=, corr=, corr_full=, miss=);
  ...
  data samp_1 samp_2 samp_3;
    ...
    where &miss. = 0;   /* keep only non-missing per your scenario flag */
  ...
  data samp_1_w_flags;
    ...
    /* Raw visit participation flags (before baseline missingness) */
  ...
  /*====================================================*
   | 3) Inject ~5% MCAR missingness for V1 variables    |
   |    AND update PARTICIPANT_V2/V3 to "analytic"      |
   |    flags (participation + non-missing baseline)    |
   *====================================================*/
  data samp_1_miss;
    set samp_1_w_flags;

    /* Artificial missingness in baseline predictors */
    array missvars age_base x12 x14;   /* variables to blank */
    do over missvars;
      if rand('uniform') < 0.10 then missvars = .;
    end;
...
* miss_ind_mar;
%impute_sudaan(start=1, end=200, corr=ind, corr_full=independent, miss= miss_ind_mar);
