...
proc printto log = "&homepath./logs/s4_sudaan_double_rr&sysdate..log"
			 print= "&homepath./lst/s4_sudaan_double_rr&sysdate..lst" new; run; 
...
%macro impute_sudaan(start=, end=, corr=, corr_full=, miss=);
  %do i = &start. %to &end.;
	...
	data samp;	
        ...
        bghhsub_s2_nr = bghhsub_s2/ (rr_glm*rr_nradj);
...
* miss_ind_mar;
%impute_sudaan(start=1, end=100, corr=ind, corr_full=independent, miss= miss_ind_mar);
